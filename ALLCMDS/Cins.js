...
cmd({
  ...
  try {
    if (!q) return await reply("*Please provide a search query or a direct movie link! (e.g., Deadpool or https://cinesubz.co/movies/noryang-deadly-sea-2023-sinhala-subtitles/)*");

    const isDirectLink = q.startsWith("https://cinesubz.co/movies/");

    if (isDirectLink) {
      // Handle direct movie link
      ...
      const downloadMessage = await conn.sendMessage(from, { text: linksMessage }, { quoted: message });
