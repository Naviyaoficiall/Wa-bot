...
cmd(
  ...
  async (conn, mek, m, { from, q, reply }) => {
    try {
      ...
      // Search API
      const searchUrl = `https://www.dark-yasiya-api.site/search/baiscope?text=${encodeURIComponent(
        q
      )}`;
      ...
      const handleUserResponse = async (messageUpdate) => {
        const userMek = messageUpdate.messages[0];
        if (!userMek.message) return;

        const userReply =
          userMek.message.conversation ||
          userMek.message.extendedTextMessage?.text;
        const isReplyToSentMsg =
          userMek.message.extendedTextMessage?.contextInfo?.stanzaId ===
          messageID;
        ...
        if (isReplyToSentMsg && /^[0-9]+$/.test(userReply)) {
          ...
          // Download API
          const downloadUrl = `https://www.dark-yasiya-api.site/download/baiscope?url=${encodeURIComponent(
            selectedItem.link
          )}`;
          ...
          if (!download_link) {
            return reply(
              "Subtitle download link is not available. Please try again later."
            );
          }
          ...
          conn.ev.off("messages.upsert", handleUserResponse); // Remove listener after use
        }
      };
